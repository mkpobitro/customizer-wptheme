...
function cust_assets(){
	...
	//Service Icon Color Dynamic from Customizer
	$service_icon_color = get_theme_mod( 'cust_service_icon_color', '#dd2d6a' );
	$service_style = <<<EOD
.service i {
	color: {$service_icon_color};
}
EOD;
	wp_add_inline_style( 'cust-main', $service_style );
}
add_action('wp_enqueue_scripts','cust_assets');


function cust_live_preview_assets(){
	wp_enqueue_script( 'cust-customizer-js', get_theme_file_uri('assets/js/customizer.js'), array('jquery', 'customize-preview'), time(), true );
}
add_action( 'customize_preview_init', 'cust_live_preview_assets' );
